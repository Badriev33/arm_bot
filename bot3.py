...
async def process_start_application(message: types.Message, state: FSMContext):
  ...
  if user_id in blocked_users:
    ...
    await message.reply(f"Сбор заявок в данный момент окончен. Свободных мест на основной танцпол не осталось, но вы можете взять с собой контроллер и ноутбук, и в случае если кто-то из диджеев не приедет - будет свободное местечко.\n\nP.S. Если ты играешь лёгкий жанр, то ты можешь обратиться к организатору чилл-зоны, чтобы встать на второй танцпол:\n@G_0_T_L")
...
async def process_location(message: types.Message, state: FSMContext):
  ...
  if user_id in blocked_users:
    ...
    # channel_chat_id = -1002218574693
    # message_ids = [3, 2, 4]  # ID сообщений с фотографиями

    # media_group = []
    # for msg_id in message_ids:
    #     msg = await bot.forward_message(chat_id=channel_chat_id, from_chat_id=channel_chat_id, message_id=msg_id)
    #     if msg.photo:
    ...
    channel_chat_id = -1002218574693
    message_ids = [2, 3, 4]  # ID сообщений с фотографиями

    media_group = []
    for idx, msg_id in enumerate(message_ids):
        msg = await bot.forward_message(chat_id=1310388442, from_chat_id=channel_chat_id, message_id=msg_id)
        if msg.photo:
            photo_file_id = msg.photo[-1].file_id
            if idx == 0 and msg.caption:
                media_group.append(types.InputMediaPhoto(media=photo_file_id, caption=msg.caption))
            else:
                media_group.append(types.InputMediaPhoto(media=photo_file_id))
